  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <Star
          key={i}
          className={`w-4 h-4 ${i <= Math.floor(value) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-600'}`}
        />
      ))}
    </div>
  );
          ...
          <section className="relative py-20 overflow-hidden">
            ...
            <div className="container mx-auto px-6 relative z-10">
              ...
              <nav className="flex items-center gap-2 text-sm text-gray-500 mb-8">
                ...
                <Link href="/outils" className="hover:text-purple-400 transition-colors">Outils</Link>
                <ChevronRight className="w-4 h-4" />
          ...
          <div className="container mx-auto px-6 pb-32">
            <div className="flex flex-col lg:flex-row gap-10">
              ...
              <aside className="lg:w-64 flex-shrink-0">
                <div className="gradient-card rounded-2xl p-5 sticky top-24">
                  <h3 className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-4">Catégories</h3>
                  <ul className="space-y-1">
                    <li>
                      <Link
                        href="/outils"
                        className="flex items-center justify-between px-3 py-2.5 rounded-xl text-sm text-gray-400 hover:text-white hover:bg-purple-900/30 transition-all group"
                      >
              ...
              <div className="flex-1 min-w-0">
                ...
                {meta.longDesc && (
                  <div className={`gradient-card border ${meta.border} rounded-2xl p-6 mb-8`}>
                    <p className="text-gray-300 leading-relaxed">{meta.longDesc}</p>
                  </div>
                )}
                ...
                {tools.length > 0 ? (
                  ...
                  <div className="text-center py-20 gradient-card rounded-2xl">
                    <p className="text-4xl mb-4">🔍</p>
                    <p className="text-gray-400 text-lg">Aucun outil dans cette catégorie pour l&apos;instant.</p>
                    <Link href="/outils" className="inline-block mt-4 text-purple-400 hover:text-purple-300 transition-colors">
                ...
                {allCategories.filter(c => c.slug !== slug).length > 0 && (
                  <div className="mt-16">
                    <h2 className="text-xl font-bold text-white mb-6">Autres catégories</h2>
                        ...
                        .map(cat => {
                          ...
                          return (
                            <Link
                              key={cat.slug}
                              href={`/outils/${cat.slug}`}
                              className="gradient-card rounded-2xl p-5 flex items-center gap-4 hover:-translate-y-1 hover:border-purple-500/40 transition-all duration-300 group"
                            >
                              ...
                              <ChevronRight className="w-4 h-4 text-gray-600 group-hover:text-purple-400 transition-colors ml-auto" />
                            </Link>
                          );
                        })}
                    </div>
