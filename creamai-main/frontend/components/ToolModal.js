...
export default function ToolModal({ tool, onClose }) {
  ...
  return (
    ...
    >
      ...
      {/* Modal Content */}
      <div className="relative bg-[#0a0118] border border-purple-500/30 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        {/* Close Button */}
        <button 
          onClick={onClose}
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                ...
                {tool.verified && (
                  <span className="bg-green-900/30 border border-green-500/30 text-green-300 px-3 py-1 rounded-full text-xs font-semibold">
                    ✓ Vérifié
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {(tool.categories || []).map((cat, idx) => (
                  <span 
                    key={idx}
                    className="bg-purple-900/30 border border-purple-500/30 text-purple-300 px-3 py-1 rounded-full text-sm"
                  >
                    {cat}
                  </span>
                ))}
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {tool.strengths.map((strength, idx) => (
                  <li key={idx} className="flex items-start gap-2">
                    ...
                    <span className="text-gray-300">{strength.replace(/__/g, '')}</span>
                  </li>
                ))}
              </ul>
            </div>
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {tool.limitations.map((limitation, idx) => (
                  <li key={idx} className="flex items-start gap-2">
                    ...
                    <span className="text-gray-300">{limitation.replace(/__/g, '')}</span>
                  </li>
                ))}
              </ul>
            </div>
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {tool.idealFor.map((item, idx) => (
                  <li key={idx} className="flex items-start gap-2">
                    ...
                    <span className="text-gray-300">{item.replace(/__/g, '')}</span>
                  </li>
                ))}
              </ul>
            </div>
              ...
              <div className="space-y-4">
                {tool.faq.slice(0, 5).map((item, idx) => (
                  <div key={idx} className="bg-[#110822]/50 rounded-xl p-4">
                    <h4 className="font-semibold mb-2 text-purple-300">{item.q}</h4>
