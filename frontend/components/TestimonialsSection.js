    ...
    <section className="py-20 bg-gradient-to-b from-transparent via-purple-900/5 to-transparent">
      <div className="container mx-auto px-6">
        <div className="text-center mb-14">
          ...
          <p className="text-gray-900 max-w-lg mx-auto">Des entrepreneurs et créateurs qui ont trouvé les bons outils grâce à Thecreamai.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <div key={i} className="gradient-card rounded-2xl p-6 hover:-translate-y-2 hover:border-purple-500/50 transition-all">
              <div className="text-gray-900 text-lg mb-4">{'★'.repeat(t.stars)}</div>
              <p className="text-gray-600 text-sm leading-relaxed italic mb-6">"{t.text}"</p>
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${t.color} flex items-center justify-center font-bold text-gray-900`}>{t.initial}</div>
                <div>
