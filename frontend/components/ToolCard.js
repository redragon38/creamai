...
=======
    <div className={`bg-white rounded-2xl flex flex-col overflow-hidden border-2 ${meta.border} hover:-translate-y-2 hover:shadow-xl transition-all duration-300 group cursor-pointer shadow-sm`}>
      {/* Barre colorée top */}
      <div className={`h-1.5 w-full ${meta.topBar}`} />
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="w-14 h-14 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center overflow-hidden shadow-sm flex-shrink-0">
            {tool.logo ? (
...
=======
            {tool.verified && <span className="bg-green-50 border border-green-200 text-green-700 px-2 py-0.5 rounded-full text-xs font-bold">✓ Vérifié</span>}
            {tool.featured && <span className="bg-yellow-50 border border-yellow-300 text-yellow-700 px-2 py-0.5 rounded-full text-xs font-bold">🏆 Top</span>}
            {tool.trial && <span className="bg-cyan-50 border border-cyan-200 text-cyan-700 px-2 py-0.5 rounded-full text-xs font-bold">🆓 Essai</span>}
>>>>>>> c228359ea51c899aa80248fc4488d9fe7c10d9f7
          </div>
        </div>
...
=======
          <h3 className="text-lg font-bold text-gray-900 mb-1 group-hover:text-purple-700 transition-colors">{tool.name}</h3>
>>>>>>> c228359ea51c899aa80248fc4488d9fe7c10d9f7
        </Link>
...
=======
        <p className="text-gray-500 text-sm leading-relaxed line-clamp-2 flex-1 mb-4">{tool.short || tool.highlight || ''}</p>

        {/* Note */}
        {tool.rating && (
          <div className="flex items-center gap-2 mb-4">
            <Stars val={tool.rating.value} />
            <span className="text-xs text-gray-500">{tool.rating.value} ({tool.rating.count} avis)</span>
          </div>
        )}
        ...
        <div className="flex gap-2 mt-auto">
          ...
          <a href={url} target="_blank" rel="noopener noreferrer"
            className="flex-1 gradient-purple text-white py-2 rounded-xl font-semibold text-xs flex items-center justify-center gap-1 hover:shadow-md hover:shadow-purple-300/40 transition-all">
>>>>>>> c228359ea51c899aa80248fc4488d9fe7c10d9f7
